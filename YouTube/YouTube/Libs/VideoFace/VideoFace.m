...
/**
 获取视频中的音频，生成音频文件保存沙盒
 **/
+(BOOL)getAudioForVideoAsset:(AVURLAsset *)avasset name:(NSString *)name cachePath:(NSString *)cachePath {
    if (name == NULL || [name isEqualToString:@""]) {
        NSLog(@"导出音频文件失败----%@", cachePath);
        return NO;
    }
    ...
    NSString *outPutFilePath = [cachePath stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.m4a", name]];
    session.outputURL = [NSURL fileURLWithPath:outPutFilePath];
    session.outputFileType = AVFileTypeAppleM4A;
    [session exportAsynchronouslyWithCompletionHandler:^{
        NSLog(@"导出音频文件完成m4a----%@", outPutFilePath);
//        [LameTool audioCAFtoMP3:outPutFilePath];
    }];
    return YES;
}
...
+ (void)splitVideo:(AVURLAsset *)avasset cachePath:(NSString *)cachePath ACompletedBlock:(void (^)(NSMutableArray *times, float fps))completionBlock{
    ...
    //生成音频保存到沙盒
    [self getAudioForVideoAsset:avasset name:@"音频" cachePath:cachePath];
}
